...
import quarry.http_server as hs
...
class TestFdTelemetry:
    """Daemon open-fd telemetry (_log_fd_usage): count/pct math + WARNING gate."""

    @staticmethod
    def _patch_fds(
        monkeypatch: pytest.MonkeyPatch, *, open_fds: int, soft_limit: int
    ) -> None:
        """Force _log_fd_usage to observe *open_fds* against *soft_limit*."""
        monkeypatch.setattr(
            "quarry.http_server.Path.iterdir",
            lambda _self: iter(range(open_fds)),
        )
        monkeypatch.setattr(
            "quarry.http_server.resource.getrlimit",
