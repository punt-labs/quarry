...
\begin{faqpair}{What is the scaling story?}
  ...
  Initial indexing is the main time investment. The ONNX embedding model
  processes \texttildelow8 chunks/second per thread on an M2 CPU; with two
  concurrent workers, a 10{,}000-file directory (\texttildelow70{,}000 chunks)
  takes roughly 1.5--2 hours. This is comparable to macOS Spotlight, which
  re-indexes for hours after a long offline period. Files are queryable as
  they are indexed---the user does not wait for the full sync to complete.
  Subsequent syncs are incremental and fast (only new or changed files).
