...
\date{March 2026 (implementation status revised September 2026)}
\hypersetup{
...
\begin{enumerate}
  ...
  \item \textbf{WebFetch auto-capture}: URLs fetched during a session
    are automatically ingested into the project's
    \texttt{<repo>-captures} collection (\texttt{default-captures} when
    the working directory is not registered).
...
ingested into the project's \texttt{<repo>-captures} collection
(async, fire-and-forget; the daemon scrubs before storing).
...
the project's \texttt{<repo>-captures} collection (as
\texttt{session-<id8>}, attributed to the repo's ethos pin). This
must happen before \texttt{CompactContext}.
...
PostToolUse & WebFetch & CaptureWebFetch &
  Ingest URL & No \\
PostToolUse & WebSearch$^\ddagger$ & CompleteTool\-Success (base) &
  Capture search digest & No \\
PostToolUse & Read$^\ddagger$ & CompleteTool\-Success (base) &
  Capture read file (opt-in) & No \\
...
SubagentStop$^\ddagger$ & * & StopSubagent (base) &
  Archive transcript, distill report & No$^\S$ \\
SessionEnd & * & EndSession &
  Capture full transcript, clear state & No \\
\hline
\end{tabular}
}
...
\texttt{mcp\_\_(plugin\_quarry(-dev)?\_)?quarry(-proxy|-dev)?\_\_.*}
(see \texttt{plugin/hooks/hooks.json}).\\
$^\ddagger$The capture side-effect rides a base-layer operation; the
quarry layer does not model it as a separate schema. SubagentStop
files the subagent's own final report as an \texttt{observation} in
\texttt{memory-<handle>} when \texttt{agent\_type} names a registered
ethos identity (DES-055).\\
$^\S$Claude Code treats SubagentStop as a blocking hook (a non-zero
exit or a \texttt{decision} field keeps the subagent running); the
handler returns \texttt{\{\}} on every path, so it never blocks.
...
All seven capture handlers are implemented in
\texttt{src/quarry/hooks.py} (session start, web fetch, pre-compact)
and \texttt{src/quarry/hooks\_agent.py} (session end, web search,
read, subagent stop), dispatched by \texttt{quarry-hook <event>}
(\texttt{src/quarry/\_hook\_entry.py}) and wired via shell scripts in
\texttt{plugin/hooks/hooks.json}.  Every handler is a thin daemon
client: no hook loads the engine.
...
Web search capture & PostToolUse (WebSearch) &
  web-search.sh, hooks\_agent.py & Correct \\
Read capture (opt-in) & PostToolUse (Read) &
  post-read.sh, hooks\_agent.py, read\_capture.py & Correct \\
Compaction capture & PreCompact &
  pre-compact.sh, hooks.py & Correct \\
Session-end capture & SessionEnd &
  session-end.sh, hooks\_agent.py & Correct \\
Subagent capture + distillation & SubagentStop &
  subagent-stop.sh, hooks\_agent.py, subagent\_capture.py & Correct \\
\hline
\end{tabular}
}
...
StopSubagent (capture side-effect) & hooks\_agent.py + subagent-stop.sh & Correct \\
EndSession & hooks\_agent.py + session-end.sh & Correct \\
\hline
\end{tabular}

\noindent
The \texttt{EndSession} schema models the state clear; the wired
handler additionally captures the full session transcript on every
close, so a short session that never compacts still yields a durable
capture.  The WebSearch and Read captures ride the base
\texttt{CompleteToolSuccess} operation and are listed in the hook
event map above rather than as quarry-layer schemas.
