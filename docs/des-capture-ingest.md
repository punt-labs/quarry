...
- The dedup behavior stays. If a URL is already captured in the target collection, it's skipped. The simplest way to keep this is to have the hook say "don't overwrite" and let the daemon's existing document check suppress the duplicate — no new mechanism.
