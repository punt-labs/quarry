...
The system is \textbf{daemon-first} (DES-031 v2.2, ACCEPTED, in
implementation): a single \texttt{quarryd} process holds the engine,
and the CLI, MCP, and library are clients that reach it over a versioned
wire protocol --- the daemon is the intermediary service layer.  As of
v2-3a the engine runs only in \texttt{quarryd} (the \texttt{quarry
serve} subcommand is retired), loopback requests are authenticated with a
mode-0600 \texttt{serve.token}, and the CLI remote paths resolve that
token through \texttt{ClientConfig}.  The full client tier
(\texttt{QuarryClient}) and \texttt{quarry mcp}-as-client are in flight
(v2-3/v2-4); until they land, the local-mode CLI and \texttt{quarry mcp}
still load the engine in-process.
...
\begin{enumerate}[label=\textbf{P\arabic*}.]
  \item \textbf{Daemon-first.}  The \texttt{quarryd} process is
    ...
    versioned REST contract (DES-031 v2.2, ACCEPTED, in
    implementation).  The prior \emph{library-first} principle --- the
    core library did all work and the surfaces were thin wrappers ---
    described the as-built state DES-031 v2 replaces.
...
Watch loop  & \modpath{daemon/watch\_loop.py}   & The \texttt{WatchLoop}:
              ...
              start/stop watching, \texttt{request\_scan}, FTS-rebuild
              coalescer.  Producer into the queue. \\
...
Watchdog    & \modpath{daemon/fs\_watchdog.py}  & The sole \texttt{watchdog}
              ...
              \texttt{PollingObserver} fallback. \\
...
collapses to one \texttt{CollectionSyncJob} (DES-034's size-gated flush keeps
the fragment budget). Per-file jobs \emph{never} rebuild the FTS index --- a
lone finalize job runs post-quiescence per \texttt{(database, collection)}, so
the daemon's persistent connection does not reopen the DES-045/quarry-0dss
descriptor leak; a resource-invariant test proves the fd count plateaus across
multiple databases. A full-queue 503 re-arms the event with capped backoff, never
a silent drop.
...
Remote      & \modpath{remote.py}       & Remote server config:
              mcp-proxy config read/write, TOFU CA cert fetch
              and storage, connection validation. \\
Remote client & \modpath{remote\_client.py} & \texttt{RemoteClient} ---
              authenticated HTTP client the CLI uses to route data
              commands to a remote daemon over pinned-CA HTTPS. \\
...
rather than a bare 401.  \texttt{ClientConfig} is the client tier's first
piece; the full \texttt{QuarryClient} and \texttt{quarry
mcp}-as-client land in v2-3/v2-4.
...
\begin{enumerate}
  ...
  \item Forwards to \texttt{wss://localhost:8420/mcp} over WebSocket
    (TLS, pinned CA --- even on localhost).
...
Under DES-031 v2 (ACCEPTED, in implementation) the CLI is a thin
client: it builds a request, calls the daemon over the wire protocol
via \texttt{QuarryClient}, and renders the response --- it does not
load the embedding model or open LanceDB in-process.  The prior design
had \texttt{quarry find "margins"} load the model in-process and query
LanceDB directly; that local-engine path is removed as the data
commands migrate to \texttt{QuarryClient}.
...
\begin{enumerate}
  \item \textbf{Vector similarity} --- ANN search via LanceDB under the
    cosine metric
  \item \textbf{BM25 full-text search} --- Tantivy index on the
    \texttt{text} column, created during schema migration
\end{enumerate}
...
\textbf{FTS scoring.}  FTS rows arrive from Tantivy without a vector
distance.  Rather than displaying a synthetic $1.00$ (which made
