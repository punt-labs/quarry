...
Remote      & \modpath{remote.py}       & Remote server HTTP client:
              mcp-proxy config read/write, TOFU CA cert fetch
              and storage, connection validation. \\
\bottomrule
\end{tabular}
\end{center}
...
\texttt{/documents}        & GET    & List documents                   & Yes & sync  \\
\texttt{/documents}        & DELETE & Delete document (returns 202)    & Yes & async \\
\texttt{/collections}      & GET    & List collections                 & Yes & sync  \\
\texttt{/collections}      & DELETE & Delete collection (returns 202)  & Yes & async \\
\texttt{/registrations}    & GET    & List directory registrations      & Yes & sync  \\
\texttt{/registrations}    & POST   & Register directory (returns 202) & Yes & async \\
\texttt{/registrations}    & DELETE & Deregister directory (returns 202) & Yes & async \\
...
\texttt{/remember}         & POST   & Ingest inline content (returns 202) & Yes & async \\
\texttt{/ingest}           & POST   & Ingest URL (returns 202)         & Yes & async \\
\texttt{/sync}             & POST   & Sync registrations (returns 202) & Yes & async \\
\texttt{/tasks/\{task\_id\}} & GET  & Poll task status (unified)       & Yes & sync  \\
\texttt{/sync/\{task\_id\}}  & GET  & Alias for \texttt{/tasks/\{task\_id\}} & Yes & sync \\
\texttt{/ingest/\{task\_id\}}& GET  & Alias for \texttt{/tasks/\{task\_id\}} & Yes & sync \\
...
1559 tests across 44 files.  Three tiers plus integration:
...
\begin{itemize}
  \item All mutating endpoints use \texttt{GET /tasks/\{task\_id\}}
    for status polling.  \texttt{/sync/\{id\}} and
    \texttt{/ingest/\{id\}} are backwards-compatible aliases.
  \item \textbf{Sync} rejects concurrent requests with HTTP~409
    because a second sync against the same registrations would
    race on LanceDB writes.  All other operations allow concurrency.
