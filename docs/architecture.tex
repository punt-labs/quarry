...
The system is \textbf{daemon-first} (DES-031 v2, ACCEPTED, in
implementation): a single \texttt{quarry serve} daemon holds the
engine, and the CLI, MCP, and library are pure clients that reach it
over a versioned wire protocol --- the daemon is the intermediary
service layer.  The prior \emph{projection} strategy (a single core
library that the surfaces import directly, with no intermediary service
layer) described the as-built state DES-031 v2 replaces; today the CLI
and library still load the engine in-process on local calls, and that
path is being removed.

\subsection{Design Principles}

\begin{enumerate}[label=\textbf{P\arabic*}.]
  \item \textbf{Daemon-first.}  The \texttt{quarry serve} daemon is
    the single engine (embedding model, LanceDB, pipeline, sync
    registry); the CLI, MCP, and library are pure clients over a
    versioned REST/WebSocket contract (DES-031 v2, ACCEPTED, in
...
Daemon      & \modpath{service.py}      & \texttt{quarry serve} lifecycle
