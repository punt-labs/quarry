...
MCP Server  & \modpath{mcp\_server.py}, \modpath{mcp\_missions.py} &
              ...
              thirteen tool bodies (twelve on \texttt{McpTools}, plus
              \texttt{missions\_sync} on the sibling \texttt{MissionTools},
              all behind \modpath{mcp\_guard.py}'s \texttt{ToolGuard}) are
...
Doctor     & \modpath{doctor.py} + \modpath{doctor\_*.py} & Health checks
             ...
             configuration, daemon, sync, captures, ethos memory
             (\modpath{doctor\_memory.py}, \modpath{doctor\_ethos.py}). \\
...
\emph{stdio} server (\modpath{mcp\_server.py}) whose thirteen tool bodies
...
returning after the last post (DES-055).  Thirteen tools in all: twelve
on \texttt{McpTools} plus \texttt{missions\_sync} on the sibling
\texttt{MissionTools} (\modpath{mcp\_missions.py}), registered from one
call.
...
\texttt{/v1/captures/lookup}  & GET    & Prior capture of a URL (web-fetch loop closer) & Yes & sync  \\
...
The default run (\texttt{make test}, with the \texttt{slow} and
\texttt{embedding} tiers deselected) collected 4{,}041 tests on
2026-09-19 (\texttt{uv run pytest -{}-collect-only -q}).  Three tiers
plus integration; the files named are representative, not exhaustive:
