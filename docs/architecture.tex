...
\midrule
\texttt{/health}       & GET  & Server uptime and status         & No  & sync \\
\texttt{/ca.crt}       & GET  & CA certificate PEM (TOFU)        & No  & sync \\
\texttt{/search}       & GET  & Hybrid search                    & Yes & sync \\
\texttt{/show}         & GET  & Document metadata or page text   & Yes & sync \\
\texttt{/documents}    & GET, DELETE & List or delete documents   & Yes & sync \\
\texttt{/collections}  & GET, DELETE & List or delete collections & Yes & sync \\
\texttt{/registrations}& GET, POST, DELETE & Manage directory registrations & Yes & sync \\
\texttt{/databases}    & GET  & List named databases             & Yes & sync \\
\texttt{/status}       & GET  & Database statistics               & Yes & sync \\
\texttt{/use}          & POST & Switch active database           & Yes & sync \\
\texttt{/remember}     & POST & Ingest inline content            & Yes & sync \\
\texttt{/ingest}       & POST & Ingest URL (returns 202)         & Yes & async \\
\texttt{/ingest/\{id\}}& GET  & Poll ingest task status          & Yes & --- \\
\texttt{/sync}         & POST & Sync registrations (returns 202) & Yes & async \\
\texttt{/sync/\{id\}}  & GET  & Poll sync task status            & Yes & --- \\
\texttt{/mcp}          & WS   & MCP JSON-RPC over WebSocket      & Yes & --- \\
\bottomrule
\end{tabular}
\end{center}

See Appendix~\ref{app:concurrency} for the full concurrency matrix
across HTTP, CLI, and MCP surfaces.
...
API (daemon), the CLI, and the MCP server (stdio or WebSocket).  The
concurrency model differs by surface because each has different
constraints: HTTP connections time out, MCP tool calls block the LLM
response stream, and CLI commands run in a user's terminal.
...
\midrule
%
% --- Write operations (short, sync acceptable) ---
\multicolumn{5}{l}{\emph{Short write operations}} \\
\midrule
register            & sync   & sync & sync & fire-and-forget \\
deregister          & sync   & sync & sync & fire-and-forget \\
delete (document)   & sync   & sync & sync & fire-and-forget \\
delete (collection) & sync   & sync & sync & fire-and-forget \\
use (switch db)     & sync   & sync & sync & sync \\
\midrule
%
% --- Long-running operations ---
\multicolumn{5}{l}{\emph{Long-running operations}} \\
\midrule
ingest (URL)        & fire-and-forget & sync & fire-and-forget
                    & fire-and-forget \\
ingest (file)       & ---    & sync & ---  & fire-and-forget \\
remember            & sync   & sync & sync & fire-and-forget \\
...
\textbf{Short writes} (register, deregister, delete) are synchronous
on HTTP and CLI because they complete in milliseconds.  MCP uses
fire-and-forget to avoid blocking the LLM response stream, even for
fast operations --- the bounded thread pool (4~workers) is cheap
insurance.

\textbf{Long-running operations} must be fire-and-forget on HTTP
because the CLI client has a finite connection timeout.  Sitemap
crawls, multi-file syncs, and large-document embedding routinely
exceed 15~seconds.

\begin{itemize}
  \item \textbf{Ingest} and \textbf{sync} use the 202~Accepted
    pattern: \texttt{POST} returns a \field{task\_id} immediately;
    \texttt{GET /ingest/\{task\_id\}} or
    \texttt{GET /sync/\{task\_id\}} returns
    \texttt{running}/\texttt{completed}/\texttt{failed} status.
  \item \textbf{Sync} rejects concurrent requests with HTTP~409
    because a second sync against the same registrations would
    race on LanceDB writes.
  \item \textbf{Ingest} allows concurrent requests --- each operates
    on independent documents and gets its own task.
  \item \textbf{Remember} remains synchronous on HTTP because inline
    content is already in memory; no network fetch is required.  If
    embedding latency becomes a concern for very large inputs, it
    should follow the same 202 pattern.
  \item \textbf{Optimize} is CLI-only (no HTTP or MCP surface).  It
    runs LanceDB compaction and index rebuilds, which are
    CPU-intensive and should not be triggered remotely without
    explicit intent.
\end{itemize}

\subsection{Task Lifecycle}

Background tasks (ingest, sync) follow the same lifecycle:
