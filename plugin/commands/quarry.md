...
- **missions sync**: Call `mcp__quarry__missions_sync`. Files each frozen ethos mission round of this repo into the worker's memory; run it after `ethos mission close`. Pass `mission` when a mission id follows, `dry_run: true` for `--dry-run`, and `force: true` for `--force`.
